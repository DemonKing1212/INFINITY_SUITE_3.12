...
class UdfpsAnimation : SettingsPreferenceFragment() {
    ...
    private val udfpsPkg = "com.infinity.udfps.animations"
    ...
    private fun loadResources() {
        ...
        for (i in 0 until count) {
            ...
            // Pre-load the preview drawable right here to populate the cache!
            getDrawableByName(previews[i])
        }
    }
    ...
    private fun getDrawableByName(drawableName: String): Drawable? {
        return drawableCache.getOrPut(drawableName) {
            val res = udfpsRes ?: return@getOrPut null
            val id = res.getIdentifier(drawableName, "drawable", udfpsPkg)
            if (id == 0) return@getOrPut null
            try {
    ...
    private class UdfpsAnimAdapter(
        ...
        private var selectedPos: Int = Settings.System.getIntForUser(
            contentResolver,
            Settings.System.UDFPS_ANIM_STYLE,
            0,
            UserHandle.USER_CURRENT
